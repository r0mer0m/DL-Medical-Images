...
for N in SAMPLE_AMOUNTS:
    ...
    save_path = SAVE_DIRECTORY/f"pneumonia-regular-training-{N}.pth"

    regular_training(EPOCHS, train_dl, valid_dl, model, max_lr=.001, save_path=save_path)
    
    model = DenseNet121(1, pretrained=PRETRAINED, freeze=FREEZE).cuda()

    save_path = SAVE_DIRECTORY/f"pneumonia-one-cycle-training-{N}.pth"
...
for i, N in enumerate(SAMPLE_AMOUNTS):
    ...
    load_path = SAVE_DIRECTORY/f"pneumonia-regular-training-{N}.pth"
    ...
    load_path = SAVE_DIRECTORY/f"pneumonia-one-cycle-training-{N}.pth"
...
with open('results/pneumonia_regular_training.json', 'w') as f:
    f.write(regular_training)

one_cycle_training = json.dumps(one_cycle_training)
with open('results/pneumonia_one_cycle_training.json', 'w') as f:
